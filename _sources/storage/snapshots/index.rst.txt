...
.. Important::
	A snapshot is **NOT** a :doc:`backup </backup/index>`. While it might seem like a good approach to just take recurring snapshots, so that its possible to revert to an older stage, no data is moved so in the event of an issue with the underlying platform, all snapshots would be lost. 
