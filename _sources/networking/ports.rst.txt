...
We recommend sticking to a single port (or interface) on an instance as its generally easier to understand the network flow and plan security that way. :doc:`Routing <virtual-router/routing-between-networks>` should instead be used. If using this network design then adding ports  manually should rarely be needed as an instance will have a port created when launched.

Below we will however show how to create ports in the various tools.

.. Note::
	Should you opt not to use :doc:`virtual-router/private-subnet/dhcp` to assign IP configuration to a new port (interface), keep in mind that the :doc:`mtu` is also set via DHCP and needs to be set manually in these cases. 
...
- Name your port. 
- Select the :doc:`private network <virtual-router/private-network/index>` you want to connect to under "Network".
...
- Select the :doc:`private subnet <virtual-router/private-subnet/index>` you want to use. 
- Press "create".
