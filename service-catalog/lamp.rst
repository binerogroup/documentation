...
- Under SSH Admin IP, add your public IP (this will allow your workstation through the firewall) in :doc:`CIDR notation </networking/virtual-router/private-subnet/subnet-format>`. If you need to change this later, this sets up a :doc:`security group </networking/virtual-router/security-groups/index>` on the instance. The security group can be changed or updated as you see fit but if you want to login over the floating IP that will be setup, you will need to input your IP here. 
...
- Under "local_net", select the :doc:`network </networking/virtual-router/private-subnet/index>` on which you want to run the service.
