...
- When it says "verify resize" (and a small triangle), login to the instance and verify that it worked as intended (meaning the instance is working as before but with the new flavor). 
- If you are happy with the result, press the small "check mark" in the top right corner. This will finalise the process and make it permanent.
- If you are unhappy (for some reason the process failed), you are able to instead press the small "x". This will revert the process.
