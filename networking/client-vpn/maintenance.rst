...
.. Note::
	The server will reboot after the update. Its advisable to first :doc:`snapshot </storage/snapshots/index>` the server in the event the update should fail or something should not work afterwards. 
...
.. Important::
	The "OpenVPN User" certificate is used when using the :doc:`default mode <user-modes>` user setup. When renewing this certificate, new :doc:`client configurations <logging-in>` will need to be setup in the VPN users client softwares. The "OpenVPN Server" certificate will not require new client configs but renewing it will **restart the service** which will disconnect active clients. 

.. Note::
	User certificates will have a 10 year life-span. In the event they expire, they will also need to be renewed. This will also require new VPN config for the users who use their certificates.
