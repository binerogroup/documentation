...
clients. The scope is setup on a :doc:`private subnet <index>` when creating it, instructions are provided
under each management tool.
...
VPN is setup on the same subnet that does not do :doc:`../nat`. See below for how to add routing to a DHCP pool.
...
- Under **Destination CIDR**, enter the destination network in
  :doc:`CIDR notation </networking/router/private-subnet/subnet-format>`.
...
- Enter the route as ``destination subnet in `CIDR notation </networking/router/private-subnet/subnet-format>`,nexthop (gateway)``,
