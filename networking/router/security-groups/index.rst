...
.. Note::
	An instance that does not have a floating IP connected and sits behind a router is not reachable from the internet and is therefore not (as) vulnerable. A floating IP will use :doc:`../nat` to map a public IP to the instance real IP (which is not globally routed). Its good practice to not have floating IPs on instances that does not need to be directly reachable from the internet (because they host internal services - like databases) but this will require a :doc:`a vpn or a bounce server </networking/reaching-your-instances>` to manage the instance.
