...
.. Note::
	If your intent was to revert to a snapshot so as to bring the instance or volume back to the state of taking the snapshot, please see our :doc:`/images/launch-instance-from-image` guide which does this.
...
.. Note::
	If you are unsure of which volume snapshot to delete, its possible to press it and then press its related volumes and in turn, the volumes related instances. The image is just meta-data and has no ties the the actual instance it was modeled from.
...
.. Note::
	If you are unsure of which volume snapshot to delete, its possible to press the ID of the Volume to see which instance its attached to. The image is just meta-data and has no ties the the actual instance it was modeled from.
